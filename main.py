...
class Game :
    ...
    def Play(self):  
        nb_turn = 1
        if (self.mode == 2 and self.is_playing == 2) : #Fait le premier tour de l'ordi puis passe la main au Joueur
            self.Turn(self.is_playing,nb_turn)
            self.is_playing-=1
        while self.check_winner() == 0 :
            self.Turn(self.is_playing,nb_turn)
            self.is_playing = 2 if self.is_playing == 1 else 1
            nb_turn += 1
